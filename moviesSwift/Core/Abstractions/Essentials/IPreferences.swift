...
public protocol IPreferences {
    ...
    func ContainsKey(_ key: String, sharedName: String?) -> Bool

    /// Removes a key and its associated value if it exists.
    func Remove(_ key: String, sharedName: String?)

    /// Clears all keys and values.
    func Clear(_ sharedName: String?)

    /// Sets a value for a given key.
    func Set<T>(_ key: String, value: T, sharedName: String?)

    /// Gets the value for a given key, or the default specified if the key does not exist.
    func Get<T>(_ key: String, defaultValue: T, sharedName: String?) -> T
}
