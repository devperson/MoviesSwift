...
public struct VersionInfo: Comparable, Equatable {
    ...
    public init(major: Int, minor: Int, build: Int = -1, revision: Int = -1) {
        ...
        if build != -1 { precondition(build >= 0, "Build cannot be negative.") }
        if revision != -1 { precondition(revision >= 0, "Revision cannot be negative.") }
        self.major = major
        self.minor = minor
        self.build = build
    ...
    public var majorRevision: Int16 { Int16((revision >> 16) & 0xFFFF) }
    public var minorRevision: Int16 { Int16(revision & 0xFFFF) }
    ...
    public static func ParseVersion(_ version: String) -> VersionInfo {
        if let parsed = tryParse(version) { return parsed }
        if let major = Int(version) { return VersionInfo(major: major, minor: 0) }
    ...
    public static func parse(_ input: String) -> VersionInfo {
        ...
        func parsePart(_ s: String, name: String) -> Int {
            guard let v = Int(s), v >= 0 else { fatalError("Invalid number in version part: \(name) = \(s)") }
            return v
        }
    ...
    public static func tryParse(_ input: String?) -> VersionInfo? {
        ...
        return try? { VersionInfo.parse(input) }()
    }
}
