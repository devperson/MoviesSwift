...
public struct Rect: Equatable, Hashable {
    ...
    public func Equals(_ other: Rect) -> Bool { X == other.X && Y == other.Y && Width == other.Width && Height == other.Height }

    public func equals(_ other: Any?) -> Bool {
        guard let other = other else { return false }
        if let r = other as? Rect { return Equals(r) }
        if let r = other as? Rectangle { return X == r.X && Y == r.Y && Width == r.Width && Height == r.Height }
        return false
    }
    ...
    public func Contains(rect: Rect) -> Bool { X <= rect.X && Right >= rect.Right && Y <= rect.Y && Bottom >= rect.Bottom }
    public func Contains(pt: Point) -> Bool { Contains(x: pt.X, y: pt.Y) }
    public func Contains(x: Double, y: Double) -> Bool { (x >= Left) && (x < Right) && (y >= Top) && (y < Bottom) }
    public func IntersectsWith(r: Rect) -> Bool { !((Left >= r.Right) || (Right <= r.Left) || (Top >= r.Bottom) || (Bottom <= r.Top)) }
    ...
    // Inflate and Offset
    public func Inflate(sz: Size) -> Rect { Inflate(width: sz.Width, height: sz.Height) }
    ...
    public func Offset(dx: Double, dy: Double) -> Rect { var r = self; r.X += dx; r.Y += dy; return r }
    public func Offset(dr: Point) -> Rect { Offset(dx: dr.X, dy: dr.Y) }
    public func Round() -> Rect { Rect(X: Foundation.round(X), Y: Foundation.round(Y), Width: Foundation.round(Width), Height: Foundation.round(Height)) }

    public func Deconstruct() -> DeconstructedRect { DeconstructedRect(x: X, y: Y, width: Width, height: Height) }
    public func toString() -> String { return "{X=\(X) Y=\(Y) Width=\(Width) Height=\(Height)}" }
}
