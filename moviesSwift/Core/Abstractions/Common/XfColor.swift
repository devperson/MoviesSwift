...
public final class XfColor: Equatable, Hashable, CustomStringConvertible
{
    ...
    {
        self.init(Double(r) / 255.0, Double(g) / 255.0, Double(b) / 255.0, 1.0, .Rgb)
    }

    // Kotlin: private constructor(r: Int, g: Int, b: Int, a: Int)
    private convenience init(r: Int, g: Int, b: Int, a: Int)
    {
        self.init(Double(r) / 255.0, Double(g) / 255.0, Double(b) / 255.0, Double(a) / 255.0, .Rgb)
    }
    ...
    func MultiplyAlpha(_ alpha: Double) -> XfColor
    {
        switch _mode
        {
            ...
            case .Rgb:
                return XfColor(Double(_r), Double(_g), Double(_b), Double(_a) * alpha, .Rgb)
            case .Hsl:
                return XfColor(Double(_hue), Double(_saturation), Double(_luminosity), Double(_a) * alpha, .Hsl)
        }
    }
    ...
    func AddLuminosity(_ delta: Double) -> XfColor
    {
        ...
        return XfColor(Double(_hue), Double(_saturation), Double(_luminosity) + delta, Double(_a), .Hsl)
    }
    ...
    func WithHue(_ hue: Double) -> XfColor
    {
        ...
        return XfColor(hue, Double(_saturation), Double(_luminosity), Double(_a), .Hsl)
    }
    ...
    func WithSaturation(_ saturation: Double) -> XfColor
    {
        ...
        return XfColor(Double(_hue), saturation, Double(_luminosity), Double(_a), .Hsl)
    }
    ...
    func WithLuminosity(_ luminosity: Double) -> XfColor
    {
        ...
        return XfColor(Double(_hue), Double(_saturation), luminosity, Double(_a), .Hsl)
    }
    ...
    func ToHex() -> String
    {
        ...
        {
            let intVal = Int((value * 255.0).rounded()).clamped(to: 0...255)
    ...
    {
        return FromHsva(Double(h) / 360.0, Double(s) / 100.0, Double(v) / 100.0, Double(a) / 100.0)
    }

    static func FromHsv(_ h: Int, _ s: Int, _ v: Int) -> XfColor
    {
        return FromHsva(Double(h) / 360.0, Double(s) / 100.0, Double(v) / 100.0, 1.0)
    }
    ...
    private static func ToHex(_ c: Character) -> UInt
    {
        ...
        if scalarValue >= UnicodeScalar("0").value && scalarValue <= UnicodeScalar("9").value
        {
            return UInt(scalarValue - UnicodeScalar("0").value)
        }

        let lower = scalarValue | 0x20 // make it lowercase
        if lower >= UnicodeScalar("a").value && lower <= UnicodeScalar("f").value
        {
